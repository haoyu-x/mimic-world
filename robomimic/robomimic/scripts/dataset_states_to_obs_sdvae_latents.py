...
def load_vae(pretrained_path: str, device: str = "cuda", mixed_precision: str = "bf16", batch_size: int = 64):
    """
    Load Stable Diffusion VAE from pretrained weights and set global variables.
    
    Args:
        pretrained_path: HuggingFace model name or local path (e.g., "stabilityai/sd-vae-ft-mse")
        device: Device to load model on ("cuda" or "cpu")
        mixed_precision: Precision mode ("bf16", "fp16", or "no")
        batch_size: Batch size for encoding frames
    """
    global VAE_MODEL, VAE_DEVICE, VAE_DTYPE, VAE_BATCH_SIZE, VAE_SCALING_FACTOR
    
    VAE_DEVICE = torch.device(device if torch.cuda.is_available() else "cpu")
    VAE_DTYPE = torch.bfloat16 if mixed_precision == "bf16" else (torch.float16 if mixed_precision == "fp16" else torch.float32)
    VAE_BATCH_SIZE = batch_size
    
    print(f"Loading SD VAE from: {pretrained_path}")
    print(f"Using device: {VAE_DEVICE}, dtype: {VAE_DTYPE}")
    
    VAE_MODEL = AutoencoderKL.from_pretrained(pretrained_path).to(VAE_DEVICE)
    VAE_MODEL = VAE_MODEL.to(dtype=VAE_DTYPE)
    VAE_MODEL.eval()
    
    # Get scaling factor from config (try different access methods for diffusers compatibility)
    if hasattr(VAE_MODEL.config, 'scaling_factor'):
        VAE_SCALING_FACTOR = VAE_MODEL.config.scaling_factor
    elif 'scaling_factor' in VAE_MODEL.config:
        VAE_SCALING_FACTOR = VAE_MODEL.config['scaling_factor']
    else:
        # Print available config keys for debugging
        print(f"Available config keys: {list(VAE_MODEL.config.keys()) if hasattr(VAE_MODEL.config, 'keys') else dir(VAE_MODEL.config)}")
        # Standard SD VAE scaling factor (1 / 0.18215 ≈ 5.49, used in SD training)
        VAE_SCALING_FACTOR = 0.18215
        print(f"Warning: scaling_factor not found in config, using standard SD VAE value")
    print(f"VAE scaling factor: {VAE_SCALING_FACTOR}")
    
    print("SD VAE loaded successfully.")
...
@torch.no_grad()
def sdvae_extract(video_frames):
    """
    Helper function to extract latent codes from video frames using the SDVAE model.
    
    Args:
        video_frames: numpy array of shape (T, H, W, C) with values in [0, 255] (uint8)
    
    Returns:
        latents: numpy array of shape (T, C_latent, H_latent, W_latent)
    """
    global VAE_MODEL, VAE_DEVICE, VAE_DTYPE, VAE_BATCH_SIZE, VAE_SCALING_FACTOR
    
    if VAE_MODEL is None:
        raise RuntimeError("VAE model not loaded. Call load_vae() first.")
    
    num_frames = video_frames.shape[0]
    all_latents = []
    
    # Convert from numpy (T, H, W, C) uint8 [0, 255] to torch (T, C, H, W) float [-1, 1]
    video_tensor = torch.from_numpy(video_frames).permute(0, 3, 1, 2).float()  # (T, C, H, W)
    video_tensor = video_tensor / 255.0  # [0, 1]
    video_tensor = video_tensor * 2.0 - 1.0  # [-1, 1] (SD VAE expected range)
    
    # Process in batches to avoid OOM
    num_batches = (num_frames + VAE_BATCH_SIZE - 1) // VAE_BATCH_SIZE
    for i in tqdm(range(0, num_frames, VAE_BATCH_SIZE), total=num_batches, desc="Extracting SDVAE latents", leave=False):
        end_idx = min(i + VAE_BATCH_SIZE, num_frames)
        batch_video = video_tensor[i:end_idx].to(VAE_DEVICE, dtype=VAE_DTYPE)
        
        # Encode using SD VAE
        batch_latents = VAE_MODEL.encode(batch_video).latent_dist.sample()
        
        # Scale latents by scaling_factor
        batch_latents = batch_latents * VAE_SCALING_FACTOR
        
        all_latents.append(batch_latents.cpu().float().numpy())
    
    # Concatenate all latents
    latents = np.concatenate(all_latents, axis=0)  # (T, C_latent, H_latent, W_latent)
    return latents
...
def dataset_states_to_obs(args):
    ...
    # Load SD VAE for latent extraction
    load_vae(
    ...
    for ind in tqdm(range(len(demos))):
        ...
        for k in traj["obs"]:
            print(k)
            if "image" in k:
                print("Extracting latent codes for video frames")
                print(k)
                print(traj["obs"][k].shape)
                print(traj["next_obs"][k].max(), traj["next_obs"][k].min())
                print("--------------------------------")

                video_frames = traj["obs"][k]
                video_latent = sdvae_extract(video_frames)

                if args.compress:
                    ep_data_grp.create_dataset("obs/{}".format(k), data=np.array(traj["obs"][k]), compression="gzip")
                    ep_data_grp.create_dataset("obs/{}".format(k + "_latent"), data=np.array(video_latent), compression="gzip")
                else:
                    ep_data_grp.create_dataset("obs/{}".format(k), data=np.array(traj["obs"][k]))
                    ep_data_grp.create_dataset("obs/{}".format(k + "_latent"), data=np.array(video_latent))
                if not args.exclude_next_obs:
                    if args.compress:
                        ep_data_grp.create_dataset("next_obs/{}".format(k), data=np.array(traj["next_obs"][k]), compression="gzip")
                        ep_data_grp.create_dataset("next_obs/{}".format(k + "_latent"), data=np.array(video_latent), compression="gzip")
                    else:
                        ep_data_grp.create_dataset("next_obs/{}".format(k), data=np.array(traj["next_obs"][k]))
                        ep_data_grp.create_dataset("next_obs/{}".format(k + "_latent"), data=np.array(video_latent))    
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="HuggingFace model name or local path to SD VAE (e.g., 'stabilityai/sd-vae-ft-mse', 'stabilityai/sd-vae-ft-ema')",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda",
        help="Device to use for VAE encoding (cuda or cpu)",
    )
    ...
    parser.add_argument(
        ...
        help="Mixed precision mode for VAE encoding (no = fp32)",
    )
